    ...
    with torch.set_grad_enabled(train):
        for batch in loader:
            ...
            for f in frames:
                ...
                selected_frames.append(f[idx])

            # Stack into tensor [B, 3, 224, 224]
            x = torch.stack(selected_frames).to(device)
...
def main():
    ...
    if a.mode == "spatial":
        ...
        ds_train = FrameVideoCustomDataset(root_dir=a.data_root, split="train",
                                 transform=training_transform, stack_frames=False)
        ds_val = FrameVideoCustomDataset(root_dir=a.data_root, split="val",
