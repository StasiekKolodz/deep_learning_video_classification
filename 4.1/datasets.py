...
class FlowVideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # if self.transform:
        #     flows = [self.transform(flow) for flow in video_flows]
        # else:
        #     flows = [T.ToTensor()(flow) for flow in video_flows]
        
        # flows = torch.stack(flows).permute(1, 0, 2, 3)


        return video_flows, label
    
    def load_flows(self, flows_dir):
        flows = []
        for i in range(1, self.n_flows+1):
            flow_file = os.path.join(flows_dir, f'flow_{i}_{i+1}.npy')
            flow = np.load(flow_file)  # [2, H, W]
            flows.append(flow)

        # Stack along channel dim → [2*(T-1), H, W]
        stacked_flows = np.concatenate(flows, axis=0)
        stacked_flows = torch.from_numpy(stacked_flows).float()  # [1, C, H, W]

        return stacked_flows


class FrameVideoCustomDataset(torch.utils.data.Dataset):
    def __init__(self, 
    root_dir = '/work3/ppar/data/ucf101', 
    split = 'train', 
    transform = None,
