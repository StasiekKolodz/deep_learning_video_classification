...
# Import dataset
from datasets import FrameVideoCustomDataset, FlowVideoDataset, TwoStreamDataset
...
def build_test_loader(a: EvalFusionArgs):
...
def load_model(temporal_checkpoint_path: str, spatial_checkpoint_path: str, mode: str, num_classes: int, num_frames: int, device: str):
    """Load trained model from checkpoint."""
    # Create model
    
    
    temporal_model = TemporalCNN(num_classes)
    spatial_model = SpatialCNN(num_classes)
    ...
    modes = ("temporal", "spatial")
    for mode in modes:
        if mode == "temporal":
            checkpoint = torch.load(temporal_checkpoint_path, map_location=device)
            print(f"✓ Loaded model from {temporal_checkpoint_path}")
        elif mode == "spatial":
            checkpoint = torch.load(spatial_checkpoint_path, map_location=device)
            print(f"✓ Loaded model from {spatial_checkpoint_path}")

        # Handle different checkpoint formats
        if isinstance(checkpoint, dict):
            if 'model_state_dict' in checkpoint:
                models[mode].load_state_dict(checkpoint['model_state_dict'])
            elif 'state_dict' in checkpoint:
                models[mode].load_state_dict(checkpoint['state_dict'])
            else:
                models[mode].load_state_dict(checkpoint)
        else:
            models[mode].load_state_dict(checkpoint)

        models[mode].to(device)
        models[mode].eval()

    return models["spatial"], models["temporal"]
...
@torch.no_grad()
def evaluate_model(spatial_model, temporal_model,  loader, device, num_classes: int, mode) -> Dict:
    ...
    for batch_idx, (frames, flows, targets) in enumerate(loader):
        frames = frames.to(device)
        flows = flows.to(device)
        targets = targets.to(device)

        # Spatial
        batch_spatial_logits = []
        for f in frames:  # iterate over videos in batch
            # Stack all frames of this video
            logits = spatial_model(f)  # [num_frames, num_classes]
            logits = logits.mean(dim=0)  # average over frames -> [num_classes]
            batch_spatial_logits.append(logits)
        x_spatial = torch.stack(batch_spatial_logits)  # [B, num_classes]
        spatial_probs = torch.softmax(x_spatial, dim=1) 
        
        # Temporal
       
        x_temporal = temporal_model(flows) # [B, num_classes]
        temporal_probs = torch.softmax(x_temporal, dim=1)

  
        fusion_probs = spatial_probs + temporal_probs/2
        fusion_logits = torch.log(fusion_probs + 1e-8)  # optional if you want to keep in logit space
        # Forward pass
        
        loss = F.cross_entropy(fusion_logits, targets)
        ...
        total_correct_top5 += compute_topk_accuracy(fusion_logits, targets, k=5)
...
def main():
    ...
    parser.add_argument("--temporal_checkpoint", required=True, help="Path to tmeporal model checkpoint (.pth)")
    parser.add_argument("--spatial_checkpoint", required=True, help="Path to spatial model checkpoint (.pth)")
    parser.add_argument("--data_root", default="/dtu/datasets1/02516/ufc10", help="Root of dataset")
    parser.add_argument("--mode", required=True, choices=["fusion"], 
    ...
    # Model
    if eval_args.mode == "fusion":
        print("SPATIAL DATASET")
        spatial_ds_test = FrameVideoCustomDataset(root_dir=eval_args.data_root, split="test",
                                    transform=transform, stack_frames=False)
        
        temporal_ds_test = FlowVideoDataset(root_dir=eval_args.data_root, split="test",
                                    transform=transform)
        
        ds_test = TwoStreamDataset(spatial_ds_test, temporal_ds_test)

        frame_sample1, flow_sample1, label1 = ds_test[0]

        print(f"Spatial shapes: {frame_sample1.shape}")
        print("Flow sample shape:", flow_sample1.shape)
        print("Label:", label1)

        print("Dataset size : ", len(ds_test))
    ...
    spatial_model, temporal_model = load_model(
        eval_args.temporal_checkpoint,
        eval_args.spatial_checkpoint ,
        eval_args.mode, 
    ...
    # Evaluate
    results = evaluate_model(spatial_model, temporal_model, test_loader, device, eval_args.num_classes, eval_args.mode)
