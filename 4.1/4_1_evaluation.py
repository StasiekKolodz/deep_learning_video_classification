...
def load_model(checkpoint_path: str, mode: str, num_classes: int, num_frames: int, device: str):
    ...
    # Create model
    if mode == "perframe":
        model = PerFrameAggregationModel(num_classes)
    elif mode == "late":
        model = LateFusionModel(num_classes)
    elif mode == "early":
        model = EarlyFusion2DModel(num_frames, num_classes)
    elif mode == "3d":
        model = R3D18Model(num_classes)
    else:
...
@torch.no_grad()
def evaluate_model(model, loader, device, num_classes: int) -> Dict:
...
def main():
    ...
    parser.add_argument("--mode", required=True, choices=["perframe", "late", "early", "3d"], 
    ...
    test_loader, test_dataset = build_test_loader(eval_args)
    print(f"✓ Test set size: {len(test_dataset)} samples")
    
    # Try to get class names from dataset
    class_names = None
    if hasattr(test_dataset, 'df') and 'class_name' in test_dataset.df.columns:
        # Get unique class names sorted by label
        df_classes = test_dataset.df[['label', 'class_name']].drop_duplicates().sort_values('label')
    ...
    # Evaluate
    results = evaluate_model(model, test_loader, device, eval_args.num_classes)
